...
class AppTest {
    ...
    void testGetDirectorySize() throws Exception {
        long size1 = Files.size(Path.of("src/test/resources/dir/anotherFile").toAbsolutePath());
        long size2 = Files.size(Path.of("src/test/resources/dir/firstFille").toAbsolutePath());
        long size3 = Files.size(Path.of("src/test/resources/dir/secondFile").toAbsolutePath());

        long expectedTotalSize = size1 + size2 + size3;

        CompletableFuture<Long> result = App.getDirectorySize("src/test/resources/dir");
        long actualSize = result.get();

        assertThat(actualSize).isEqualTo(expectedTotalSize);
    }
    // END
}
